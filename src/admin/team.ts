...
function absenceModal(root: HTMLElement, analyst: Analyst) {
  const today = new Date().toLocaleDateString('sv-SE');
  openModal('Definir ausência', `<form id="absenceForm" class="absence-form"><div class="absence-intro"><span class="absence-intro-icon">A</span><div><strong>${escapeHtml(analyst.name)}</strong><span>Será ocultado das escalas somente durante o período definido.</span></div></div><div class="absence-form-section"><div class="field absence-reason-field"><label for="absenceReason">Motivo da ausência <b>*</b></label><select id="absenceReason" required><option value="vacation">Férias</option><option value="medical_leave">Atestado</option></select></div><div class="absence-date-grid"><div class="field"><label for="absenceStart">Data de saída <b>*</b></label><input id="absenceStart" type="date" value="${today}" required></div><span class="absence-date-arrow" aria-hidden="true">→</span><div class="field"><label for="absenceReturn">Data de retorno <b>*</b></label><input id="absenceReturn" type="date" min="${today}" required></div></div><p class="absence-date-help">Na data de retorno, o analista volta a aparecer automaticamente nas escalas.</p></div><div class="modal-actions absence-actions"><button type="button" class="btn-ghost" data-cancel>Cancelar</button><button type="submit" class="btn-primary">Confirmar ausência</button></div></form>`, (body)=>{

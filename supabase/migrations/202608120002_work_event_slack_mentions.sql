-- Inclui o member ID do Slack no retorno usado pela Edge Function de ponto.
create or replace function public.record_my_work_event(p_event_type text) returns jsonb
language plpgsql security definer set search_path=public as $$
declare v_analyst uuid; v_name text; v_slack_user_id text; v_last text; v_lunch_at timestamptz; v_now timestamptz:=now(); v_active integer;
begin
  select p.analyst_id,a.name,a.slack_user_id into v_analyst,v_name,v_slack_user_id from profiles p join analysts a on a.id=p.analyst_id where p.id=auth.uid();
